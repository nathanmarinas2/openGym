...
const COPY = {
  en: {
    title: 'Daily briefing', subtitle: 'One view for today’s training, fuel and recovery.', training: 'Training today', planned: 'Planned', completed: 'Completed', rest: 'Rest day', volume: 'Volume', sets: 'sets', effort: 'Effort', nutrition: 'Fuel remaining', calories: 'Calories', protein: 'Protein', hydration: 'Hydration', fasting: 'Fasting', active: 'Active', inactive: 'Not active', body: 'Body trend', noWeight: 'No weight logged', recommendation: 'One useful next step', openWorkout: 'Open workout', openNutrition: 'Open nutrition', openProgress: 'Review progress', openCoach: 'Review coach', logWater: 'Log water', back: 'Home', noPlan: 'No session planned' },
  es: {
    title: 'Resumen diario', subtitle: 'Una vista para el entrenamiento, la nutrición y la recuperación de hoy.', training: 'Entrenamiento de hoy', planned: 'Planificado', completed: 'Completado', rest: 'Día de descanso', volume: 'Volumen', sets: 'series', effort: 'Esfuerzo', nutrition: 'Combustible restante', calories: 'Calorías', protein: 'Proteína', hydration: 'Hidratación', fasting: 'Ayuno', active: 'Activo', inactive: 'No activo', body: 'Tendencia corporal', noWeight: 'No hay peso registrado', recommendation: 'Siguiente paso útil', openWorkout: 'Abrir entrenamiento', openNutrition: 'Abrir nutrición', openProgress: 'Ver progreso', openCoach: 'Ver coach', logWater: 'Registrar agua', back: 'Inicio', noPlan: 'No hay sesión planificada' }
}
...
export default function Briefing() {
  ...
  const { workout, nutrition, hydration, fasting, weight, recommendation } = briefing
  const action = recommendation.type === 'training' ? () => nav('/workout') : recommendation.type === 'hydration' ? () => nav('/nutrition') : recommendation.type === 'nutrition' ? () => nav('/nutrition') : recommendation.type === 'recovery' ? () => nav('/stats') : recommendation.type === 'consistency' ? () => nav('/coach') : () => nav('/nutrition')
  return <div className="narrow briefing-view">
    ...
    <section className="card briefing-section"><div className="row between"><h2>{C.training}</h2><Icon name="dumbbell" className="nutrition-card-icon" /></div><div className="briefing-primary-value">{workout.name || (workout.planned ? workout.plannedName : C.rest)}</div><div className="briefing-status"><span className={workout.completed ? 'good' : workout.planned ? 'pending' : 'muted'}>{workout.completed ? C.completed : workout.planned ? C.planned : C.noPlan}</span>{workout.exercises > 0 && <span>{workout.exercises} exercises · {workout.sets} {C.sets}</span>}</div><div className="briefing-metric-grid"><div><span>{C.volume}</span><strong>{nice(workout.volume)}</strong></div><div><span>{C.effort}</span><strong>{workout.averageRir != null ? `${nice(workout.averageRir)} RIR` : workout.averageRpe != null ? `${nice(workout.averageRpe)} RPE` : '—'}</strong></div><div><span>{C.sets}</span><strong>{workout.sets}</strong></div></div></section>
