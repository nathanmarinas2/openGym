...
import {
  ...
  healthScore, localCoachInsights, lookupBarcode, normalizeFoodGrams, nutritionPeriod, nutritionPeriodSummary, recipeAsFood,
...
const EN = {
  ...
  barcode: 'Barcode', lookup: 'Look up', barcodeRequired: 'Enter at least 6 digits to look up a barcode.', scan: 'Scan', stopScanner: 'Close scanner', scannerUnsupported: 'Camera scanning is not supported here. Enter the barcode manually.', scannerError: 'Could not access the camera. Check browser permissions.', filters: 'Filters', hideFilters: 'Hide filters',
...
const ES = {
  ...
  barcode: 'Código de barras', lookup: 'Consultar', barcodeRequired: 'Introduce al menos 6 dígitos para consultar un código de barras.', scan: 'Escanear', stopScanner: 'Cerrar lector', scannerUnsupported: 'El escaneo con cámara no está disponible aquí. Introduce el código manualmente.', scannerError: 'No se pudo acceder a la cámara. Comprueba los permisos del navegador.', filters: 'Filtros', hideFilters: 'Ocultar filtros',
...
function FoodScore({ C, food }) {
  ...
  return <details className={`nutrition-score-detail ${score.tone}`}>
    <summary><span className={`nutrition-score-pill ${score.tone}`} aria-label={`${C.healthScore}: ${score.score}`}>{score.score}</span><span>{C.healthScore} · {label}</span><Icon name="chevronDown" className="nutrition-score-chevron" /></summary>
    <ScorePanel C={C} score={score} />
  </details>
}

function nutritionGradeTone(value) {
  const grade = String(value || '').trim().toLowerCase()
  return ['a', 'b', 'c', 'd', 'e'].includes(grade) ? grade : 'neutral'
}
...
function FoodResults({ C, results, grams, setGrams, addFood, openProduct, preferences = {}, favorites, toggleFavorite }) {
  ...
  return <div className="nutrition-results" aria-live="polite"><div className="small muted nutrition-results-title">{C.results}</div>{results.map(food => { const score = healthScore(food, preferences); const label = score ? (score.tone === 'good' ? C.scoreGood : score.tone === 'moderate' ? C.scoreModerate : C.scoreLow) : C.scoreNoData; const favorite = visibleFavoriteIds.has(food.id); const selectedGrams = grams[food.id] ?? 100; const grade = nutritionGradeTone(food.grade); return <article className="nutrition-food nutrition-food-result" key={food.id}><button type="button" className="nutrition-food-open" onClick={() => openProduct(food, results)} aria-label={`${C.viewProduct}: ${food.name}`}><span className="nutrition-food-icon">{food.image ? <img src={food.image} alt="" loading="lazy" /> : <Icon name="plate" />}</span><span className="nutrition-food-main"><span className="nutrition-food-name">{food.name}</span><span className="small muted nutrition-food-meta">{food.brand && <span>{food.brand}</span>}<span>{nice(food.per100.calories)} {C.caloriesShort} · {nice(food.per100.protein)}g {C.proteinShort}</span>{food.grade && <span className={`nutrition-grade-badge grade-${grade}`} title={`${C.scoreNutri}: ${String(food.grade).toUpperCase()}`} aria-label={`${C.scoreNutri}: ${String(food.grade).toUpperCase()}`}>Nutri {String(food.grade).toUpperCase()}</span>}</span><span className="nutrition-food-score-preview">{score && <span className={`nutrition-score-pill ${score.tone}`} aria-label={`${C.healthScore}: ${score.score}`}>{score.score}</span>}<span className="nutrition-score-preview-copy">{C.healthScore} · {label}</span><Icon name="chevronRight" /></span></span></button><div className="nutrition-add-controls"><button type="button" className={'iconbtn nutrition-favorite' + (favorite ? ' on' : '')} onClick={() => onToggleFavorite(food)} aria-label={favorite ? C.unfavorite : C.favorite}><Icon name={favorite ? 'starFill' : 'star'} /></button><div className="nutrition-amount"><NumberField ref={node => { if (node) amountRefs.current.set(food.id, node); else amountRefs.current.delete(food.id) }} value={selectedGrams} decimal={false} aria-label={`${C.grams} ${food.name}`} onChange={value => setGrams(g => ({ ...g, [food.id]: value }))} /><span>g</span></div><Button size="xs" variant="primary" icon="plus" onClick={() => addFood(food, undefined, normalizeFoodGrams(amountRefs.current.get(food.id)?.value ?? selectedGrams))}>{C.add}</Button></div></article> })}</div>
}
...
function NutritionDiary({ C, S, date, setDate, meal, setMeal, dayEntries, totals, goal, update, addFood, removeEntry, recentFoods, favoriteFoods = [], preferences = {}, favorites = [], toggleFavorite = () => {}, restoreSearch = null }) {
  ...
  const runBarcode = async (value = barcode) => {
    ...
    if (code.length < 6) {
      setError(C.barcodeRequired)
      barcodeRef.current?.focus()
      return
    }
    setLoading(true); setError('')
    try { const food = await lookupBarcode(code); setResults([food]); setQuery(food.name) }
    catch (e) { setError(e.message || C.notFound) }
    finally { setLoading(false) }
  }
  const saveManual = () => {
...
export default function Nutrition() {
  ...
  const addFood = (food, selectedMeal = meal, selectedGrams = 100) => update(s => { s.nutritionEntries = [...(s.nutritionEntries || []), { id: idOf('nutrition'), date, meal: selectedMeal, grams: normalizeFoodGrams(selectedGrams), food }] })
