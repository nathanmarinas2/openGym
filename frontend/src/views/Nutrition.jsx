...
const EN = {
  ...
  recentFoods: 'Recent foods', barcodeHint: 'Barcode lookup needs an internet connection.', localSource: 'Values can vary by product; check the product label.', preferences: 'Personal preferences', diet: 'Diet', none: 'No preference', vegetarian: 'Vegetarian', vegan: 'Vegan', allergens: 'Avoid allergens', allergensHint: 'Comma-separated, e.g. milk, peanuts', avoidAdditives: 'Flag declared additives', savePreferences: 'Save preferences', favorite: 'Favorite', unfavorite: 'Remove favorite', duplicate: 'Duplicate meal',
...
function NutritionDiary({ C, S, date, setDate, meal, setMeal, dayEntries, totals, goal, update, addFood, removeEntry, recentFoods, preferences = {}, favorites = [], toggleFavorite = () => {} }) {
...
export function NutritionCoach({ C, S, date, totals, goal, update }) {
  ...
  const [objective, setObjective] = useState(storedProfile.objective || 'performance')
  ...
  useEffect(() => { setObjective(storedProfile.objective || 'performance'); setNotes(storedProfile.notes || '') }, [storedProfile.objective, storedProfile.notes])
  ...
  const ask = async () => {
    ...
    try {
      ...
      setSource(response.source || 'local')
    } catch {
      setReview(null)
      setAnswer(C.coachFallback)
      setSource('local')
    }
    finally { setLoading(false) }
  }
  ...
  return <>
    <section className="card nutrition-coach-hero"><div className="row"><span className="nutrition-coach-orb"><Icon name="sparkles" /></span><div><h2>{C.coachTitle}</h2><p className="muted small">{C.coachSubtitle}</p></div></div><p className="nutrition-coach-scope"><Icon name="info" /> {C.coachScope}</p></section>
    <section className="card nutrition-coach-profile"><h2>{C.coachObjective}</h2><div className="nutrition-coach-objective-grid">{objectiveOptions.map(([value, label]) => <button type="button" key={value} className={`chip${objective === value ? ' on' : ''}`} onClick={() => { setObjective(value); saveProfile({ objective: value }) }}>{label}</button>)}</div><label className="nutrition-coach-notes"><span>{C.coachNotes}</span><TextField value={notes} placeholder={C.coachNotesPlaceholder} maxLength={500} onChange={event => { setNotes(event.target.value); saveProfile({ notes: event.target.value }) }} /></label></section>
...
function NutritionProductHero({ C, food, score }) {
  const nutrients = [
