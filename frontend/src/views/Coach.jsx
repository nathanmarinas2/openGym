...
function localPlanDraft(S) {
  ...
  return { schema: 'liftnex-plan-draft-v1', title: 'Local plan draft', rationale: 'A conservative draft based on your current routine templates. Review every change before applying it.', routines, cycle: null, warnings: ['No connected Coach provider was available; this is a local draft.'], confidence: 'low' }
}
...
function CoachPlanBuilder({ S, update, C }) {
  ...
  return <section className="card coach-plan-builder">
    <div className="row between"><div><h2 style={{ margin: 0 }}>{C.coachPlanTitle || 'Create a plan draft'}</h2><p className="muted small">{C.coachPlanHint || 'Coach can propose routines and cycles; nothing changes until you confirm it.'}</p></div><Icon name="clipboard" style={{ color: 'var(--acc)' }} /></div>
    {!S.aiConsent && <div className="nutrition-ai-notice"><span>{C.coachConsent}</span></div>}
    <Button variant="primary" icon="sparkles" disabled={!S.aiConsent || loading || !(S.routines || []).length} onClick={create}>{loading ? (C.coachGenerating || 'Generating…') : draft ? (C.coachRefresh || 'Refresh draft') : (C.coachPlanButton || 'Generate draft')}</Button>
    {source && <div className="small dim" style={{ marginTop: 8 }}>{source === 'local' ? (C.coachProviderLocal || 'Local LiftNex draft') : (C.coachProviderConnected || 'Connected AI draft')}</div>}
    {diff?.valid && <div className="coach-plan-diff" style={{ marginTop: 12 }}><h3>{draft.title}</h3><p className="muted small">{draft.rationale}</p>{diff.changes.map(change => <label key={change.key} className="row" style={{ alignItems: 'flex-start', gap: 8, padding: '9px 0', borderBottom: '1px solid var(--sep)' }}><Check checked={selected.includes(change.key)} onChange={value => setSelected(current => value ? [...new Set([...current, change.key])] : current.filter(key => key !== change.key))} /><span><b>{change.title}</b><br /><span className="small muted">{change.type === 'add_routine' ? 'Routine' : 'Cycle'} · {change.description}</span></span></label>)}{draft.warnings?.length > 0 && <div className="small" style={{ color: 'var(--yellow)', marginTop: 10 }}>{draft.warnings.join(' ')}</div>}<Button variant="tinted" icon="check" disabled={!selected.length} onClick={apply}>{C.coachApply || 'Apply selected changes'}</Button></div>}
    {(S.coachSnapshots || []).length > 0 && <><h3 className="sec">{C.coachRevert || 'Reversible plan snapshots'}</h3><div className="list">{S.coachSnapshots.slice(0, 5).map(snapshot => <div key={snapshot.id} className="item"><div className="grow"><div className="tt">{snapshot.reason}</div><div className="ss">{snapshot.createdAt.slice(0, 10)}</div></div><Button size="sm" variant="tinted" onClick={() => revert(snapshot.id)}>{C.coachUndo || 'Revert'}</Button></div>)}</div></>}
  </section>
}
