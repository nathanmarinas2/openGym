...
export default function Stats() {
  ...
  if (curEx) {
    S.workouts.forEach(w => {
      ...
      if (en) { const sets = en.sets.filter(s => s.done && isWorkingSet(s)); const mx = Math.max(0, ...sets.map(metric), curCardio || curTimed ? 0 : (en.topW || 0)); if (mx > 0) { exPts.push({ t: w.start, y: mx, d: w.d, sets, target: en.target }); if (mx > exBest) exBest = mx } }
    })
