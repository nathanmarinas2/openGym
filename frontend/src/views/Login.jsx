...
function RegisterSheet({ close }) {
  ...
  return <>
    ...
    <input ref={ref} className="input" aria-label={t('Your name')} placeholder={t('Your name')} maxLength={40} value={name} onChange={e => setName(e.target.value)} />
    {inviteOnly && <>
      <div style={{ height: 10 }} />
  ...
  return (
    <div className="narrow" style={wrap}>
      ...
      {webauthnOK() ? <>
        ...
        <div style={{ height: 10 }} />
      </> : <div className="card small muted" style={{ textAlign: 'left' }}>{t("This browser doesn't support passkeys — you can still use LiftNex locally on this device.")}</div>}
