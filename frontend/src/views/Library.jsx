...
import { activeProfile, availableExercise } from '../lib/equipment.js'
...
export default function Library({ embedded = false }) {
  ...
  const base = allExercises(S).filter(e => (!bp || e.bp === bp) && (!availableOnly || availableExercise(S, e)) && (!ql || e.n.toLowerCase().includes(ql) || e.tg.includes(ql) || e.eq.includes(ql) || (e.desc || '').toLowerCase().includes(ql)))
  ...
  return <>
    {!embedded && <div className="hdr"><div><h1>{t('Exercises')}</h1><div className="sub">{t('{0} exercises with animations', EXDB.length)}</div></div></div>}
    ...
    <div className="chips" style={{ marginBottom: eqOpts.length > 1 ? 8 : 12 }}>
      ...
      <button className={'chip nocap' + (availableOnly ? ' on' : '')} onClick={() => { setAvailableOnly(v => !v); setEq(''); setShown(40) }}>{t('Available: {0}', profile.name)}</button>
    ...
    <div className="list">
      <Tappable className="item" onClick={() => customExSheet(null, ex => exerciseDetailSheet(ex), q.trim())}>
        <div className="thumb thumb-x"><Icon name="sparkles" /></div>
        <div className="grow"><div className="tt">{t('Create your own exercise')}</div><div className="ss">{t('name + body part, no animation')}</div></div><Icon name="plus" className="chev" />
      </Tappable>
      {f.slice(0, shown).map(e => {
        const best = bestWeightFor(S, e.id)
        return <Tappable key={e.id} className="item" onClick={() => exerciseDetailSheet(e)}>
          <Thumb ex={e} />
