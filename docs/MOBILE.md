...
LiftNex ships in two flavors from the same codebase:
...
- **iOS:** a Mac with Xcode 15+ and CocoaPods (`brew install cocoapods`). A free Apple ID
  ...
  for App Store distribution, which LiftNex doesn't do.
...
LiftNex's mobile app is not on the Play Store or App Store, and that's a choice: no store
...
keytool -genkeypair -keystore my.keystore -alias liftnex -keyalg RSA -validity 10950

# align + sign (zipalign/apksigner ship with the Android SDK build-tools)
zipalign -f -p 4 app-release-unsigned.apk aligned.apk
apksigner sign --ks my.keystore --ks-key-alias liftnex --out LiftNex.apk aligned.apk
```
...
- **License:** LiftNex is AGPL-3.0, which by itself sits badly with app-store terms of
